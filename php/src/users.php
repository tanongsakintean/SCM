                    ...
                    <select name="role" class="form-control">
                        ...
                        if ($result_roles->num_rows > 0) {
                            ...
                            while($r = $result_roles->fetch_assoc()) {
                                $selected = (isset($r['is_default']) && $r['is_default'] == 1) ? 'selected' : '';
    ...
    <div class="modal-box" style="width: 500px; text-align: left;">
        ...
        <div class="modal-body" style="padding: 0 30px 20px;">
            <form action="action/role_create_db.php" method="post">
                ...
                <div class="form-group" style="margin-bottom: 15px;">
                    <label class="form-label">สิทธิ์การเข้าถึงเมนู (Access Control)</label>
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px;">
                        <label><input type="checkbox" name="permissions[]" value="sales"> ฝ่ายขาย (Sales)</label>
                        <label><input type="checkbox" name="permissions[]" value="orders"> คำสั่งซื้อ (Orders)</label>
                        <label><input type="checkbox" name="permissions[]" value="approve_orders"> อนุมัติคำสั่งซื้อ (Approve)</label>
                        <label><input type="checkbox" name="permissions[]" value="receive_credit"> รับเครดิต (Receive Credit)</label>
                        <label><input type="checkbox" name="permissions[]" value="reports"> รายงาน (Reports)</label>
                        <label><input type="checkbox" name="permissions[]" value="settings"> การตั้งค่า (Settings)</label>
                        <label><input type="checkbox" name="permissions[]" value="users"> ผู้ใช้งาน (Users)</label>
                    </div>
                </div>
    ...
    <div class="modal-box" style="width: 500px; text-align: left;">
        ...
        <div class="modal-body" style="padding: 0 30px 20px;">
            <form action="action/role_update_db.php" method="post">
                ...
                <!-- Role ID/Name is now hidden as requested -->
...
<script>
    ...
    function openEditRoleModal(data) {
        ...
        fetch('action/get_role_permissions.php?role_id=' + data.role_id)
            .then(response => response.json())
            .then(permissions => {
                if (Array.isArray(permissions)) {
                    permissions.forEach(key => {
                        const cb = document.querySelector('#editRoleModal input[value="' + key + '"]');
